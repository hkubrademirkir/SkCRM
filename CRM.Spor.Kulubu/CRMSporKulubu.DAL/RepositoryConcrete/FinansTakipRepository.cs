...
namespace CRMSporKulubu.DAL.RepositoryConcrete
{
    public class FinansTakipRepository : IFinansTakipRepository
    {
		...
		public ICollection<AylikGelirGiderDTO> FinansBilgileriniGetir(string ad)
		{
			using (CRMContext _db = new CRMContext())
			{
				return (from finans in _db.FinansTakip
						...
						}).Where(x => x.KategoriAdi.Contains(ad) || x.CalisanAdi.Contains(ad) || x.SporcuAdi.Contains(ad) || x.UrunAdi.Contains (ad) || x.UyeAdi.Contains(ad)).ToList();
			}
		}
